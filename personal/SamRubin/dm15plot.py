...
cur = con.cursor()
sql_input = 'SELECT * FROM Supernovae WHERE Velocity BETWEEN -20 AND -2 AND Phase BETWEEN -2 AND 2 AND Redshift BETWEEN 0 AND .05'
X = []
Y = []
highvel = []
lowvel = []
...
for SN in SN_Array:
    if SN.velocity < -12:
        highvel.append(abs(SN.velocity))
        highdm15.append(SN.dm15)
    else:
        lowvel.append(abs(SN.velocity))
        lowdm15.append(SN.dm15)

fig = plt.figure()
plt.title(r'$\Delta m15$ vs. Line Velocity')
plt.xlabel(r'$\Delta m15$')
plt.ylabel('Velocity [10^3 km/s]')
...
"""
plt.plot(lowdm15, lowvel, 'bo', label = 'Low Velocity')
plt.plot(highdm15, highvel, 'ro', label = 'High Velocity')
plt.legend(loc = 'lower right')
plt.savefig('dm15-velocity-scatter-maxlight.png')
