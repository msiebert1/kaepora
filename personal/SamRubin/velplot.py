...
cur = con.cursor()
sql_input = 'SELECT * FROM Supernovae WHERE Velocity BETWEEN -20 AND -2 AND Phase BETWEEN-1 AND 1 AND Redshift BETWEEN 0 AND .05'
X = []
Y = []
highvel = []
lowvel = []
...
for SN in SN_Array:
    if SN.velocity < -12:
        ...
        highred.append(SN.redshift)
    else:
        lowvel.append(abs(SN.velocity))
        lowred.append(SN.redshift)

fig = plt.figure()
plt.title('Redshift vs. Line Velocity')
...
"""
plt.plot(lowred, lowvel, 'bo', label = 'Low Velocity')
plt.plot(highred, highvel, 'ro', label = 'High Velocity')
plt.legend(loc = 'lower right')
plt.savefig('redshift-velocity-scatter-maxlight.png')
