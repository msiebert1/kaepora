...
#Reads in maximum brightness spectra file names
...
def gal_comp_res(spectra_arrays):
	#Deredshift data
	parameters = Table.read('/users/malloryconlon/astr596/data/cfa/cfasnIa_param.dat',format='ascii')
...
#Read in host galaxy information

host_info=Table.read('../../MalloryConlon/Galaxy/host_info.dat',format='ascii')
sn_name_1=host_info["col1"]
type=host_info["col2"]

#Pre-allocate arrays for various galaxy host types
elliptical = []
S0 = []
spiral = []
irregular = []
...
for k in range(len(sn_name_1)):
    for l in range(len(elliptical)):
        if sn_name_1[k]==elliptical[l]:
            sn_elliptical.append(os.path()
