...
dm15_s_interp = phot_db.dm15_from_fit_params(events, salt_dict, cfa_dict, stretch='s')
dm15_x1_interp = phot_db.dm15_from_fit_params(events, salt2_dict, cfa_dict, stretch='x1')
dm15_delta_interp = phot_db.dm15_from_fit_params(events, mlcs31_dict, cfa_dict, stretch='delta')
dm15_delta_lowrv_interp = phot_db.dm15_from_fit_params(events, delt_dict, cfa_dict, stretch='delta_lowrv')
...
for event in events:
	...
	if delt_dict.get(event) != None:
		print delt_dict.get(event)
		dm15_from_delta_lowrv = float(dm15_delta_interp(delt_dict.get(event,lowrv_none)[0]))


	dm15_cfa = cfa_dict.get(event,cfa_none)[4]
	print event, dm15_from_s, dm15_from_x1, dm15_from_delta, dm15_from_delta_lowrv, dm15_cfa